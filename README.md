...
- `ios/` — iPhone controller: walk mode (on-device STT/TTS), headphone media buttons
- `android/` — Daylight DC1 viewer (read-only sessions, YubiKey-gated actions)
- `channels/` — Claude Code MCP channels (voice, grunt)
- `protocol/` — message vocabulary, envelope schema, pairing ceremony
- `spikes/` — throwaway assumption tests, never shipped

## Milestones

1. **Walk mode** — phone streams mic to Scribe directly, text + buttons over relay, TTS on phone
...
generated by a command you run yourself (values come from `~/.env`):

```sh
source ~/.env && cat > ios/Sources/Secrets.swift <<EOF
...
- `~/Coding/dictate-remote/` (car remote iOS app) stays as-is; `ios/` is its successor.
